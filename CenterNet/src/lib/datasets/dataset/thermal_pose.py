...
from utils.evalCoco_per_kpts import COCOevalPerKpt
import numpy as np
import json
import os
...
class THERMALPOSE(data.Dataset):
  num_classes = 1
  num_joints = 17
  ...
  def __init__(self, opt, split):
    super(THERMALPOSE, self).__init__()
    ...
    self.data_dir = os.path.join(opt.data_dir, 'thermal_pose')
    ...
    if split == 'test':
      ...
      self.annot_path = os.path.join(
        self.data_dir, 'annotations', 
        'thermalPose_{}.json').format(split)
    self.max_objs = 32
    ...
    print('==> initializing Thermal Pose {} data.'.format(split))
  ...
  def run_eval(self, results, save_dir):
    ...
    coco_eval = COCOevalPerKpt(self.coco, coco_dets, "keypoints")
    coco_eval.evaluate()
    coco_eval.accumulate()
    coco_eval.summarize()
    """
