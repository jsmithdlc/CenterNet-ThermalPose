...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, heads, head_conv, **kwargs):
        self.inplanes = 64
        ...
        self.heads = heads
        final_kernel = 1
        intermediate_channel = cfg['MODEL']['INTERMEDIATE_CHANNEL']
        for head in self.heads:
            ...
            if head_conv > 0:
              ...
              fc = nn.Conv2d(intermediate_channel, classes, 
                  kernel_size=1, stride=1, 
                  padding=0, bias=True)
              if 'hm' in head:
...
def get_hrpose_net(num_layers, cfg, heads, head_conv, **kwargs):
    model = PoseHigherResolutionNet(cfg,heads,head_conv,**kwargs)
